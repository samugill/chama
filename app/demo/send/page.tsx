// app/demo/seed/page.tsx
"use client";
import { useEffect, useMemo, useState } from "react";
import { getPrograms, setLastProgramId } from "@/lib/storage";
import { seedDailyLogsForDemo, clearDailyLogs } from "@/lib/storage";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-4 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto text-xs text-gray-500">영상 촬영용: 지난 7일 실행 로그를 한 번에 생성</div>
      </div>
      ...
      <div className="card p-4">
        <div className="text-sm text-gray-600">
          시드 후&nbsp;
          <a className="link" onClick={()=>router.push(`/today?program=${programId}`)}>오늘 페이지</a> /
          <a className="link ml-2" onClick={()=>router.push(`/trend`)}>추세 페이지</a>
          &nbsp;에서 결과를 확인하세요.
        </div>
      </div>
    </div>
